...
def create_task(user_id):
    ...
    day = list(map(int, input('when is the due date of this task? (yyyy,mm,dd) ').split(',')))
...
def in_calendar(user_id):
    ...
    with open("task_list.csv", 'r') as f:
        ...
        i = 1
        for line in reader:
            if line[1] == str(user_id) and line[11] == '1' and line[10] == '0':
                """
                line[1]: task owner's id
    ...
    for i in range(6):
        ...
        if month_now == 12:
            ...
            menu_res.append((month_now+i, year_now))
            month_now = 1
            year_now += 1
        else:
            print(f'{i + 1}-{month[month_now + i]} {year_now}')
    ...
    for task in calendar_task:
        if int(task[0][2][5:7]) == (month_name+1):
            due_date_list.append(int(task[0][2][8:10]))
    due_date_list = sorted(set(due_date_list))
    if is_leap(year):  # check if the year is leap
        days_in_month[1] = 29
    month_day = days_in_month[month_name]
    day_one = date(year, month_name+1, 1)  # make the date of first day of month
    ...
    for j in due_date_list:
        for task in calendar_task:
            if j == int(task[0][2][8:10]) and int(task[0][2][5:7]) == month_name+1:
                print(f'{j} --> {task[0][0]}:{task[0][1]}')
